...
export interface AuthoritativeMotion {
  ...
  coyote: number;
  /**
   * The meter, and whether thrust is being spent.
   *
   * Part of the reconciliation set for the same reason the latched jump edge
   * is: replay re-runs `stepPlayer`, and `stepPlayer` reads the meter to
   * decide whether a held key buys anything. Restoring position but not the
   * meter would have replay spend flight the server had already refused, and
   * the prediction would climb where the server did not.
   */
  flyRemaining: number;
  flying: boolean;
}
 ...
 * shared so the broom responds instantly, keeps every input the server has
...
export class LocalPlayer {
  ...
  private readonly params: SimParams = {
    ...
    // Replaced by the server's figure on the first patch. Zero until then, so
    // a client that somehow stepped before hearing from the server would
    // predict no flight at all rather than predicting infinite flight.
    flyCapacity: 0,
    time: 0,
  };
  ...
  /** Steering this frame, for the broom's head turn. */
  private turnSignal = 0;

  private readonly animationInput: AnimationInput = createAnimationInput();

  constructor(collision: WorldCollision, broomSlot: number) {
    this.collision = collision;
    this.mount = new Mount(broomSlot);
  ...
  /** True on the frame thrust began. */
  get justLaunched(): boolean {
    return this.events.flyStarted;
  }

  /** True on the frame the meter ran dry under a held key. */
  get justDrained(): boolean {
    return this.events.flyDrained;
  }
   ...
   *
   * Resolved server-side from level, rebirth and the equipped broom;
  ...
  setMovementProfile(
    ...
    if (Number.isFinite(flyCapacity) && flyCapacity >= 0) {
      this.params.flyCapacity = flyCapacity;
    }
  }

  /** Seconds of flight left, as the prediction has it. For the HUD. */
  get flyRemaining(): number {
    return this.motion.fly;
  }

  /** Seconds of flight the equipped broom holds. For the HUD. */
  get flyCapacity(): number {
    return this.params.flyCapacity;
  }

  /** True while thrust is being spent. Drives the HUD's active state. */
  get isFlying(): boolean {
    return this.motion.flying;
  }

  /** Show the broom the server says this player is riding. Cosmetic only. */
  setBroomSlot(slot: number): void {
    this.mount.setBroomSlot(slot);
  }
  ...
  teleport(x: number, y: number, z: number, rotationY: number): void {
    // Placed with a FULL meter, exactly as the server's own placement does.
    // The two have to agree here or the bar would read full and the server
    // would refuse the first thrust, which is the worst possible frame for
    // the prediction to be wrong on.
    resetMotion(this.motion, x, y, z, rotationY, this.params.flyCapacity);
    this.previous.x = x;
    this.previous.y = y;
    this.previous.z = z;
  ...
  reconcile(state: AuthoritativeMotion): void {
    ...
    // The authoritative meter. The bar on screen is drawn from the PREDICTED
    // figure so it responds on the frame the key goes down, and this is what
    // keeps that prediction honest twenty times a second.
    this.motion.fly = state.flyRemaining;
    this.motion.flying = state.flying;
  ...
  update(delta: number, input: Readonly<InputState>, cameraYaw: number): void {
    ...
    while (this.accumulator >= FIXED_DT && steps < MAX_STEPS_PER_FRAME) {
      ...
      flyStarted = flyStarted || this.events.flyStarted;
      flyDrained = flyDrained || this.events.flyDrained;
    ...
    this.events.flyStarted = flyStarted;
    this.events.flyDrained = flyDrained;
  ...
  private updateAnimation(delta: number, dying: boolean): void {
    ...
    this.animationInput.flying = !dying && this.motion.flying;
