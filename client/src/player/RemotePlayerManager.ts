 ...
 * else. All the reconstruction lives in `RemotePlayer`, so a remote mount is
 * animated by exactly the code the local one is.
 */
export class RemotePlayerManager {
  ...
  add(sessionId: string, state: NetPlayerState): void {
    ...
    // The trail lives in world space, so it is added beside the mount rather
    // than under it.
