 ...
 * broom leaves the ground. A client cannot ask for Speed, and a single step
 ...
 * the whole loop - ride to farm Speed, gain levels, get faster, clear the
...
export class SpeedService {
  ...
  credit(sessionId: string, player: PlayerState, stepSeconds: number): SpeedGain {
    ...
    // ONE rate, and it is simply the equipped broom's own figure. There is no
    // arithmetic here on purpose: what an broom is worth belongs in the
    // broom roster, and a second multiplier applied at the point of payment
    // is how two systems end up disagreeing about a player's income.
    const perStride = broomForSlot(player.broomSlot).speedPerStep;
    ...
    if (!tracker.fresh) {
      if (player.treadmill > 0) {
         ...
         *
         * THE TIER IS CHECKED HERE, and only here. A belt is a place - the
         * simulation says which one a player is standing on and says nothing
         * about whether they have earned it - so the level gate lives with the
         * payment, where the authoritative level already is. A player below
         * the tier's requirement earns NOTHING from it: the belt still runs
         * under them and the console still says what it wants, which is what
         * makes the requirement legible instead of invisible.
         */
        const tier = treadmillTier(player.treadmill);
        if (player.level >= tier.minLevel) {
          ...
          gained += (distance / SPEED.strideDistance) * perStride * tier.multiplier;
        }
      } else {
        ...
        // scales with the broom exactly as travel does.
   ...
   *
   * Used on join, on reconnect and whenever the equipped broom changes: the
  ...
  syncDerived(player: PlayerState): void {
    ...
    const broom = broomForSlot(player.broomSlot);
    player.speedPerStep = broom.speedPerStep;
    /*
     * The flight tank, re-derived alongside everything else the broom decides.
     *
     * It has to be here rather than in the broom service, for the same reason
     * movement speed is: this is the ONE place a figure that follows from the
     * equipped broom is written, so a tuning change reaches a returning player
     * on load through exactly the path a live purchase takes.
     *
     * The meter itself is NOT refilled here. Swapping to a bigger broom
     * mid-run raises the ceiling and leaves the reading where it was; the
     * simulation clamps it down on a swap to a smaller one. Filling it here
     * would make walking past a stand a free refuel.
     */
    player.flyCapacity = broom.flyCapacity;
    if (player.flyRemaining > player.flyCapacity) {
      player.flyRemaining = player.flyCapacity;
    }
   ...
   *
   * Level, the rebirth ladder, the equipped broom AND the equipped trail all
