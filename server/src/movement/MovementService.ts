...
export class MovementService {
  ...
  teleport(
    ...
    if (!sim) return;
    // Placed with a FULL tank. Every placement is a fresh start - joining,
    // dying, banking a stage, rebirthing - and sending a player back to the
    // vault on an empty meter would punish them twice for one mistake.
    resetMotion(sim.motion, x, y, z, yaw, player.flyCapacity);
    this.publish(player, sim);
  }
  ...
  applyInput(
    ...
    stepPlayer(
      ...
      sanitiseInput(message),
      {
        ...
        // The equipped broom's tank, resolved server-side. The meter is spent
        // against THIS and clamped to it every step, so a broom swap can never
        // leave a player holding more flight than they own.
        flyCapacity: player.flyCapacity,
  ...
  private publish(player: PlayerState, sim: Sim): void {
    ...
    // The meter, and whether it is currently being spent. Both come out of the
    // simulation the server just ran; neither was ever in a message.
    player.flyRemaining = m.fly;
