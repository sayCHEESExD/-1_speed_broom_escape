...
# monorepo and the server imports `@broom/shared` as a workspace dependency;
...
# imports require: `@broom/shared` resolves to `shared/dist/index.js`.
RUN npm run build:server

# Drop to production dependencies in place. This keeps the workspace symlinks
# that `@broom/shared` resolves through - deleting node_modules and
